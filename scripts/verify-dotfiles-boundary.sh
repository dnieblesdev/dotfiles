...
missing_path() {
    ...
    if [ -e "$REPO_ROOT/$path" ] || [ -L "$REPO_ROOT/$path" ]; then
        printf 'removed installer/controller surface still exists: %s\n' "$path" >&2
        return 1
    fi
}
...
removed_legacy_context_allowed() {
    local file="$1"
    local line="$2"

    awk -v line="$line" 'NR >= line - 2 && NR <= line + 2 { print }' "$file" \
        | grep -qi -E 'removed|no[[:space:]-]+(legacy[[:space:]-]+installer[[:space:]-]+)?(compatibility|shim|preservation)|no[[:space:]-]+longer[[:space:]-]+authoritative|not[[:space:]-]+authoritative|non-authoritative|no .*authoritative|must[[:space:]]+not[[:space:]]+require[[:space:]]+compatibility|without[[:space:]]+shims?|superseded|recover|history|No local'
}

stale_removed_legacy_refs() {
    local pattern="$1"
    shift
    local found=1
    ...
    while IFS= read -r match; do
        ...
        if ! removed_legacy_context_allowed "$file" "$line"; then
            printf '%s\n' "$match"
            found=0
        fi
...
if stale_removed_legacy_refs 'installer/install\.sh|installer/dotlink|cmd/bootstrap-controller|internal/controller|bootstrap-controller' \
    "$REPO_ROOT/README.md" "$REPO_ROOT/docs" "$REPO_ROOT/openspec/specs"; then
    printf 'stale authoritative installer/controller reference found\n' >&2
    exit 1
fi
...
if grep -R -n -i -E 'shell-owned behavior first|shell-owned bootstrap|shell-owned.*canonical|shell-first (bootstrap )?(canonical|canonicality|authority)|shell behavior.*resolved in favor|resolved in favor of the shell|frontend changes must preserve shell contract|shell-based installer behavior is canonical|shell bootstrap is canonical|Go/TUI surface is optional frontend behavior|Go/TUI path described as optional|Go controller is optional' \
    ...
    | grep -v -i -E 'superseded|no longer authoritative|removed|not authoritative|non-authoritative|ADR'; then
...
if grep -R -n -E 'installer/lib|catalog\.sh|planner\.sh|state\.sh|util\.sh' \
    ...
    | grep -v -E 'scripts/verify-dotfiles-boundary\.sh'; then
    printf 'stale operational installer library reference found\n' >&2
    exit 1
fi

if [ -z "$ARCHIVE_DIR" ] || [ ! -d "$ARCHIVE_DIR" ]; then
    printf 'dotfiles boundary archive evidence not found\n' >&2
    exit 1
fi

if grep -R -n -E 'installer/lib|catalog\.sh|planner\.sh|state\.sh|util\.sh' \
    "$REPO_ROOT/README.md" "$REPO_ROOT/docs" "$REPO_ROOT/openspec/specs" "$ARCHIVE_DIR" 2>/dev/null \
    | grep -v -E 'removed|Removed|no longer authoritative|not authoritative|non-authoritative|superseded|Superseded|historical|archive'; then
    printf 'stale authoritative installer library reference found\n' >&2
    exit 1
fi

printf 'dotfiles boundary verification passed\n'
